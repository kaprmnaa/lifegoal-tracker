  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="lock-settings-note">
          PIN & Face ID/Touch ID tersimpan hanya di perangkat ini, tidak dikirim ke server.
          Kalau ganti perangkat atau hapus data browser, perlu disetel ulang.
        </p>
        ...
        <div className="security-row">
          ...
          {lock.hasPin ? (
            <button className="btn-danger-ghost" style={{ width: 'auto', marginTop: 0 }} onClick={lock.removePin}>
              Nonaktifkan
            </button>
          ) : (
            <button className="btn-secondary" style={{ width: 'auto' }} onClick={startSetPin}>
              Aktifkan
            </button>
          )}
        </div>
        ...
        {(step === 'setPin1' || step === 'setPin2') && (
          <div className="pin-setup-block">
            ...
            <input
              ...
              autoFocus
            />
            <button className="btn-primary" onClick={confirmPin} disabled={busy}>
        ...
        <div className="security-row">
          ...
          {lock.biometricAvailable && (
            lock.hasBiometric ? (
              <button className="btn-danger-ghost" style={{ width: 'auto', marginTop: 0 }} onClick={lock.removeBiometric}>
                Nonaktifkan
              </button>
            ) : (
              <button className="btn-secondary" style={{ width: 'auto' }} onClick={handleEnableBiometric} disabled={busy}>
        ...
        {lock.lockEnabled && (
          <button className="btn-secondary" onClick={() => { lock.lockNow(); onClose() }}>
            Kunci Sekarang
          </button>
        )}
      </div>
